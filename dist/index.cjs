...
function* genNameEmojiString(seed) {
  ...
  const nameList = [];
  while (true) {
    const newName = "" + emojiList[rand.next().value % emojiList.length] + emojiList[rand.next().value % emojiList.length] + emojiList[rand.next().value % emojiList.length];
  ...
  minifyStyles(tree) {
    return tree.walk((node) => {
      if (node.tag === "style" && node.content) {
        const ast = safeParser__default(node.content.toString());
    ...
    return tree.walk((node) => {
      if (node.attrs && this.customAttributes.length > 0) {
        ...
        for (const attributes of this.customAttributes.filter((attr) => nodeAttrs.includes(attr))) {
          node.attrs[attributes] = node.attrs[attributes].split(/\s+/).map((value) => {
  ...
  walkRules(nodes) {
    for (const rule of nodes) {
      if (rule.type === "atrule" && (rule.name === "media" || rule.name === "supports")) {
        ...
        selectorParser__default((selectors) => {
...
const index = (options) => (tree) => new MinifyClassnames(options).process(tree);
